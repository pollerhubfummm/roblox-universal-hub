...
-- UserInputService für Tastatureingaben
...
local themes = {
    ...
    Red = {
        ...
        secondary = Color3.fromRGB(60, 35, 35),
    ...
    Blue = {
        primary = Color3.fromRGB(25, 35, 50),
        secondary = Color3.fromRGB(35, 50, 75),
...
hub.hotkey = Enum.KeyCode.RightShift
...
-- Teleport Daten
hub.data.teleportSlots = {}
hub.data.teleportHistory = {}
hub.data.deathPosition = nil
...
function hub:createGui()
    ...
    topBar.CornerRadius = UDim.new(0, 8)
    ...
    titleLabel.Text = "🎮 Roblox Universal Hub v" .. hub.version
    ...
    minimizeBtn.CornerRadius = UDim.new(0, 4)
    ...
    for i, tab in ipairs(tabs) do
        ...
        tabBtn.Position = UDim2.new(0, 4, 0, yOffset)
        ...
        tabBtn.CornerRadius = UDim.new(0, 4)
    ...
    minimizeBtn.MouseButton1Click:Connect(function()
        ...
        mainFrame.Size = hub.minimized and UDim2.new(0, 600, 0, 35) or UDim2.new(0, 600, 0, 700)
    end)
    ...
    for tabName, tabBtn in pairs(tabButtons) do
        tabBtn.MouseButton1Click:Connect(function()
            for tName, tBtn in pairs(tabButtons) do
                tBtn.BackgroundColor3 = tName == tabName and hub.currentTheme.accent or hub.currentTheme.primary
            end
...
function hub:addSlider(parent, name, min, max, default, callback, yOffset)
    ...
    slider.CornerRadius = UDim.new(0, 4)
...
function hub:showFPS()
    ...
    fpsLabel.CornerRadius = UDim.new(0, 4)
...
function hub:setupTabs()
    ...
    hub:addButton(teleportContent, "Save Current Position", function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            local posName = "Slot_" .. tostring(tick())
    ...
    hub:addButton(teleportContent, "Safe Teleport (Maus)", function()
    ...
    hub:addButton(movementContent, "Infinite Jump [ON/OFF]", function()
        hub.data.infiniteJumpEnabled = not hub.data.infiniteJumpEnabled
        if not hub.data.infiniteJumpEnabled then
            hub:enableInfiniteJump()
        end
        print("Infinite Jump: " .. tostring(hub.data.infiniteJumpEnabled))
    end, 130)
    ...
    hub:addButton(utilityContent, "Show FPS/Ping", function()
        hub:showFPS()
    end, 130)
    ...
    hub:addButton(deathReturnContent, "Enable Auto Return", function()
        if hub.data.deathPosition then
            ...
            print("Bitte erst Position speichern!")
        end
    end, 85)
    ...
    hub:addLabel(settingsContent, "Hotkey: RightShift", 280)
    hub:addLabel(settingsContent, "Press Hotkey to Hide/Show", 310)
end
...
function hub:destroy()
    print("🛑 Roblox Universal Hub beendet")
    hub.running = false
    hub.data.isFlying = false
    hub.data.isNoclipping = false
    hub.data.infiniteJumpEnabled = false
